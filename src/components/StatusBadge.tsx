...
const estados: Record<AppStatus, { emoji: string; texto: string; color: string }> = {
  inicio: { emoji: "🚀", texto: "Ha comenzado", color: "bg-secondary text-text" },
  ...
  mudanza: { emoji: "🎉", texto: "¡Nos mudamos!", color: "bg-green-100 text-green-700" },
};
...
export default function StatusBadge({ porcentaje }: { porcentaje: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold ${color}`}
    >
      <span>{emoji}</span>
      <span>{texto}</span>
    </span>
